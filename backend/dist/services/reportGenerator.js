...
function getAzureOpenAIClient() {
    ...
    if (!azureOpenAI) {
        azureOpenAI = new openai_1.AzureOpenAI({
...
async function loadTemplate(templateId) {
    let templatePath = path_1.default.join(__dirname, '../../data/templates', `${templateId}.json`);
    if (!fs_1.default.existsSync(templatePath)) {
        const sanitizedTemplateId = templateId.replace(/_/g, '-');
        templatePath = path_1.default.join(__dirname, '../../data/templates', `${sanitizedTemplateId}.json`);
    }
    if (!fs_1.default.existsSync(templatePath)) {
...
async function generateReportContent(template, applicationData, templateMappings, options) {
    const sections = [];
...
function generateBasicContent(sectionTitle, applicationData) {
...
async function enhanceContentWithAI(sectionTitle, content, applicationData) {
...
function generateHTMLReport(template, sections, applicationData, options) {
